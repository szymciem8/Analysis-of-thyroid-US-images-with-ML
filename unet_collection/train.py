...
parser.add_argument("-nf", "--nfold", default=4, type=int, help="Number of folds for cross validation")
parser.add_argument("-fi", "--fold_id", default=1, type=int, help="Current validation fold")
...
train_gen, val_gen, test_gen = get_dataset(dataset_type, batch_size, image_gen_shape, augment, nfold=nfold, fold_id=fold_id)
...
if not custom_directory:
    directory = f'{input_size}_nf{nfold}_fi{fold_id}'
else:
    directory = custom_directory
...
with strategy.scope():
    ...
    while min_loss_value > 0.5:
        ...
        min_loss_value = min(history.history['val_loss'])
